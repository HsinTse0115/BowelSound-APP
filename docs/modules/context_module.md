...
1. **預載模擬資料**：
   ...
   - 預載 4 筆腸音採集紀錄，分布於不同病患中，包含不同的錄音時間、AI 診斷結果（正常、異常），以及包含 50 個點的模擬波形數組、10x10 的模擬頻譜圖數組。
2. **新增病患與紀錄生成邏輯**：
   - `addPatient`：自動生成遞增的 ID（例如 `P-1004`），時間設定為目前 ISO 時間，並將新增的病患加入 state 中。
   - `addRecord`：傳入病患 ID、錄音時長、咖啡因、症狀、飯後時間與環境音量後，生成一筆採集紀錄（ID: `R-XXXX`）。AI 結果狀態為正常或異常，信心度在 0.8 至 0.98 之間，並產生波形與頻譜 mock data。
3. **系統設定儲存**：
   - `updateSettings`：支援局部更新設定，預設 API 伺服器網址為 `http://localhost:8000`，預設錄音時長為 10 秒，預設硬體連線狀態為 `false`。
   - 病患、檢測紀錄、系統設定與個人資料皆使用 AsyncStorage 持久化，App 啟動時載入。
...
- **輸入**：呼叫 `addRecord('P-1001', 10, false, [], '空腹', 35)`
- **預期輸出**：
  - `records` 陣列長度變為 5。
...
## 禁止事項
- 本機持久化資料必須使用明確的 storage key，並保留由使用者操作更新或清除的能力。
